<script>
  ...
  const deleteMemento = () => {
    mementoes.update((currentMementoes) => {
      return currentMementoes.filter(
        (currentMemento) => currentMemento.id !== memento.id
      );
    });
  };
</script>
    ...
    <div class="memento-wrapper">
      <div class="memento">
        ...
        <h1
          ...
          bind:textContent={memento.title}
        >
          &nbsp;
        </h1>
        <div contenteditable="true" bind:innerHTML={memento.message} />
        <button class="delete" on:click={deleteMemento}>❌</button>
      </div>
      <div class="actions">
        <button
          class="button save-button"
          on:click={() => mainController.save(memento)}>Save</button
        >
        <button
          class="button preview-button"
          on:click={() => mainController.preview(memento)}>Preview</button
        >
        <div class="options">
